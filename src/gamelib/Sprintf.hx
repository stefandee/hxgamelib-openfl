          ...
          //trace("Format: " + byte);
						...
						case "u","x","X":
            {
              ...
              var uintValue: UInt = cast( list.shift(), UInt );
							...
							if ( byte == 'x' )
							{
								value = StringTools.hex(uintValue).toLowerCase();//gamelib.Utils.hex(uintValue).toLowerCase();//uintValue.toString( 16 );
							}
							else
							if ( byte == 'X' )
							{
								value = StringTools.hex(uintValue).toUpperCase();//gamelib.Utils.hex(uintValue).toUpperCase();//uintValue.toString( 16 ).toUpperCase();
							}
							else
							{
